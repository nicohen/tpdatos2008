...
void readFile(char* filename){
	...
	if (file.is_open())
	{
		...
		//Leo la primer seccion que es la seccion de "TAMA�O". Indica el largo del string
		file.read(&lenght,SIZEOF_RECORD_SIZE_SECTION);
		//Me muevo hacia adelante para pararme luego de la seccion de "TAMA�O". Indica el largo del string
...
void Test_Block_TheClearMethodDoesIt(TestCase* test){
	...
	//test->Assert_True_m(hadSuccessRemoving(remove(filename)),"El DataBlock no cre� ningun archivo");
	fstream file (filename,ios::in|ios::binary);
	if(file.is_open()){
		file.seekg (0, ios::end);
		test->Assert_True_m(blockSize==file.tellg(),"El DataBlock creo un archivo de tama�o distinto a 512");
		...
		test->Assert_True_m(nonSpacesFound==0,"El DataBlock dej� algunos espacios no vacios");
		file.close();
	}
...
void Test_Block_DoesNotMakesEfectiveUntilTheFlushMethodIsCalled(TestCase* test){
	...
	if(file->is_open()){
		...
		test->Assert_True_m(!file->is_open(),"El flush no cre� ningun archivo");	
	}
	delete file;
	delete block;
